...
plots_direc = os.path.join(direc, 'minMaxNormalizedPlots/' + patient_direc)
...
# Create directories for each kinematic sensor position (Only create for WRA & WRB sensors)
for label in sensor_labels[:2]:
...
def generate_kinematic_plots(file):
    ...
    if "static" not in file:
        ...
        # Generate plots for only WRA & WRB sensors.
        col_filter = ['WR' in column for column in data.columns]
        selected_data = data.iloc[:, col_filter]
        selected_data_cols = selected_data.columns

        # Normalize data by rescaling the range of data to go from 0 to 1.
        ...
        for ii in range(normalized_data.shape[1]):
            plt.plot(normalized_data[selected_data_cols[ii]], 'r')
            plt.xlim(0.0)
            plt.axis('off')
            plt.savefig(plots_direc + sensor_labels[ii // 3] + '/' + file.split('.')[0] + '_' + selected_data_cols[ii] + '.png', dpi=600)
            plt.close()
