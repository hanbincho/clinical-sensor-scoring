...
patient_direc = 'Stroke/'  # set as 'Stroke/' or 'Healthy/'
...
plots_direc = os.path.join(direc, 'unProcessedPlots/' + patient_direc)
...
def generate_kinematic_plots(file):
    ...
    if "static" not in file:
        ...
        for ii in range(selected_data.shape[1]):
            plt.plot(selected_data[selected_data_cols[ii]], 'r')
            plt.xlim(0.0)
            plt.axis('off')
